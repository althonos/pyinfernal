...
project = pyinfernal.__name__
author = re.match('(.*) <.*>', pyinfernal.__author__).group(1)
year = datetime.date.today().year
copyright = '{}, {}'.format("2025" if year==2025 else "2025-{}".format(year), author)

# extract the semantic version
semver = semantic_version.Version.coerce(pyinfernal.__version__)
version = str(semver.truncate(level="patch"))
release = str(semver)

# patch the docstring of `pyinfernal` so that we don't show the link to redirect
# to the docs (we don't want to see it when reading the docs already, duh!)
doc_lines = pyinfernal.__doc__.splitlines()
...
html_theme_options = {
    ...
    "icon_links": [
        {
            "name": "GitHub",
            "url": "https://github.com/althonos/pyinfernal",
            "icon": "fa-brands fa-github",
        },
        {
            "name": "PyPI",
            "url": "https://pypi.org/project/pyinfernal",
...
htmlhelp_basename = pyinfernal.__name__
